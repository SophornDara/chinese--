...
const activeMath = computed(() => (angleMode.value === 'deg' ? mathDeg : mathRad))
...
function insert(text, type = 'value') {
  ...
  if (type === 'factorial') {
    if (expr && /[\d.)%]$/.test(last) && last !== '%') expression.value = expr + '!'
...
function handlePress(key) {
  ...
  switch (key) {
    case 'dot': insert('.', 'dot'); break
    case 'add': insert('+', 'op'); break
    case 'sub': insert('-', 'op'); break
    case 'mul': insert('*', 'op'); break
    case 'div': insert('/', 'op'); break
    case 'power': insert('^', 'op'); break
    case 'pow2': insert('^2', 'exponent'); break
    case 'pow3': insert('^3', 'exponent'); break
    case 'percent': insert('%', 'percent'); break
    case 'paren-l': insert('(', 'paren-l'); break
    case 'paren-r': insert(')', 'paren-r'); break
    case 'factorial': insert('!', 'factorial'); break
    ...
    case 'm+': mPlus(); break
    case 'm-': mMinus(); break
    case 'mr': mRecall(); break
    case 'mc': mClear(); break
    case 'sin': insert('sin(', 'fn'); break
    case 'cos': insert('cos(', 'fn'); break
    case 'tan': insert('tan(', 'fn'); break
    case 'asin': insert('asin(', 'fn'); break
    case 'acos': insert('acos(', 'fn'); break
    case 'atan': insert('atan(', 'fn'); break
    case 'ln': insert('log(', 'fn'); break
    case 'log': insert('log10(', 'fn'); break
    case 'exp': insert('exp(', 'fn'); break
    case 'pow10': insert('10^', 'fn'); break
    case 'sqrt': insert('sqrt(', 'fn'); break
    case 'cbrt': insert('cbrt(', 'fn'); break
    case 'pi': insert('pi', 'value'); break
    case 'e': insert('e', 'value'); break
    default: break
  }
}
...
function loadHistory() {
  try {
    ...
    return parsed
      .filter((item) => item && typeof item.expr === 'string' && typeof item.result === 'string')
      .slice(0, MAX_HISTORY)
  } catch (err) {
    return []
  }
...
function onKeydown(event) {
  ...
  const k = event.key
  if (/^[0-9]$/.test(k)) { insert(k, 'digit'); event.preventDefault(); return }
  switch (k) {
    case '.': insert('.', 'dot'); event.preventDefault(); break
    case '+': insert('+', 'op'); event.preventDefault(); break
    case '-': insert('-', 'op'); event.preventDefault(); break
    case '*': insert('*', 'op'); event.preventDefault(); break
    case '/': insert('/', 'op'); event.preventDefault(); break
    case '^': insert('^', 'op'); event.preventDefault(); break
    case '%': insert('%', 'percent'); event.preventDefault(); break
    case '(': insert('(', 'paren-l'); event.preventDefault(); break
    case ')': insert(')', 'paren-r'); event.preventDefault(); break
    case '!': insert('!', 'factorial'); event.preventDefault(); break
    case '=':
    case 'Enter': equals(); event.preventDefault(); break
    case 'Escape': ac(); event.preventDefault(); break
    case 'Backspace': back(); event.preventDefault(); break
    case 'Delete': ce(); event.preventDefault(); break
    default:
      if (mode.value !== 'calculus') {
        if (k === 'p') { insert('pi', 'value'); event.preventDefault() }
        else if (k === 'e') { insert('e', 'value'); event.preventDefault() }
        else if (k === 's') { insert('sin(', 'fn'); event.preventDefault() }
        else if (k === 'c') { insert('cos(', 'fn'); event.preventDefault() }
        else if (k === 't') { insert('tan(', 'fn'); event.preventDefault() }
        else if (k === 'n') { insert('log(', 'fn'); event.preventDefault() }
        else if (k === 'l') { insert('log10(', 'fn'); event.preventDefault() }
        else if (k === 'a') { insert('sqrt(', 'fn'); event.preventDefault() }
      }
  }
}
    ...
    <div class="mx-auto w-full max-w-3xl px-4 py-6">
      <header class="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            <svg v-if="dark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="h-5 w-5">
      ...
      <div class="flex flex-col gap-5 md:flex-row md:items-start">
        <div class="w-full max-w-lg flex-1 rounded-3xl bg-white p-4 shadow-xl ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-slate-700">
          <nav class="mb-5 grid grid-cols-3 gap-2 rounded-2xl bg-slate-100 p-1.5 dark:bg-slate-900">
            <button
              v-for="tab in tabs"
              :key="tab.key"
              type="button"
        ...
        <div v-if="historyOpen" class="fixed inset-0 z-30 bg-black/40 backdrop-blur-sm md:hidden" @click="historyOpen = false"></div>
        <div
          v-if="historyOpen"
        ...
        >
          <HistoryPanel :items="history" @reuse="reuseHistory" @clear="clearHistory" />
        </div>
      </div>
    </div>
